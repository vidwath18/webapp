pipeline {
	...
	stage ('SAST') {
       ...
       stage ('Deploy-To-Tomcat') {
            steps {
           sshagent(['tomcat']) {
                sh 'scp -o StrictHostKeyChecking=no target/*.war ubuntu@18.157.159.84:/home/ubuntu/prod/apache-tomcat-8.5.56/webapps/webapp.war'
              }      
           }       
    }
	    ...
	    stage ('Port Scan') {
		    steps {
			sh 'rm nmap* || true'
			sh 'docker run --rm -v "$(pwd)":/data uzyexe/nmap -sS -sV -oX nmap 18.157.159.84'
			sh 'cat nmap'
		    }
	    }
	    ...
	    stage ('DAST') {
		    	...
		    	steps {
			    sshagent(['zap']) {
				    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.85.77.1 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.157.159.84:8080/webapp/" || true'
			    }
			}
		}    
	    ...
	    stage ('Nikto Scan') {
			...
			sh 'docker run --user $(id -u):$(id -g) --rm -v $(pwd):/report -i secfigo/nikto:latest -h 18.157.159.84 -p 8080 -output /report/nikto-output.xml'
	    ...
	    stage ('SSL Checks') {
			...
			sh 'python -m sslyze --regular 18.157.159.84 --json_out sslyze-output.json'
